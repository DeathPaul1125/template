<x-app-layout>
    ...
    <div class="space-y-6" x-data="{
        ...
        types: ['string','text','integer','bigInteger','boolean','date','datetime','decimal','float','enum','image','file'],
        ...
        addField() {
            this.fields.push({ name: '', type: 'string', nullable: false, searchable: true, length: '', enum_values: '', accept: '' });
        },
        ...
        loadMeta(meta) {
            ...
            this.fields       = (meta.fields || []).map(f => ({
                ...
                length: f.length || '', enum_values: f.enum_values || '', accept: f.accept || ''
            }));
        ...
        hasAccept(t)  { return ['image','file'].includes(t); }
    }">
                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-700/20">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <input x-show="hasEnum(field.type)"
                                               ...
                                               class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-brand-500 focus:ring-brand-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300">
                                        <input x-show="hasAccept(field.type)"
                                               type="text"
                                               :name="'fields[' + index + '][accept]'"
                                               x-model="field.accept"
                                               :placeholder="field.type === 'image' ? 'jpg,png,webp,gif' : 'pdf,xlsx,docx'"
                                               class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-brand-500 focus:ring-brand-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300">
                                        <span x-show="!hasLength(field.type) && !hasEnum(field.type) && !hasAccept(field.type)"
            ...
            <details class="bg-slate-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-2xl">
                ...
                <div class="px-5 pb-4 pt-2 grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-xs">
                    @foreach([
                        ...
                        'image'      => 'Imagen (sube y guarda la ruta, ej: jpg,png)',
                        'file'       => 'Archivo adjunto (guarda la ruta, ej: pdf,xlsx)',
                    ] as $t => $desc)
